...
### 8.10 timeline_background_layers / timeline_background_periods

`timeline_background_layers` はプロジェクト、名称、説明、手動順、既定の表示状態を持つ。`timeline_background_periods` は所属レイヤー、名称、説明、色、開始・終了の歴史日付、両端の概算状態を持つ。

- プロジェクト削除またはレイヤー削除で背景期間を連鎖削除する
- 開始・終了は紀元前、年・月・日、年代、世紀、原表記、暦法を分離し、生成した正規化境界で終了が開始以降であることをDBでも保証する
- 非公開時は所有者だけ、公開時は匿名を含む閲覧者が参照でき、書き込みは常に所有者だけに限定する
- レイヤー順と期間範囲に必要なB-treeだけを作成し、検索索引や描画結果は保存しない
