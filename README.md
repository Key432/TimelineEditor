...
初期実装では「子イベント」を期間型タイムライン項目に紐づける形で実装しますが、将来的にはイベントを複数のタイムライン項目へ紐づけたり、タイムライン項目とイベント項目の間に関連線を描画したりできるように拡張します。これに備えて、将来の多対多関連と関連線描画を扱うためのデータ構造と表示レイヤーの拡張余地を初期実装から確保します。
...
`/projects/[projectId]/timeline` で、期間型・時点型の項目をサイドパネルから作成・編集し、基本時間軸上へ表示できます。本文、出典、外部URLは同じフォーム内の「詳細情報」を展開して管理できます。
...
- プロジェクト設定と対象種別管理はタイムラインを離れず右サイドパネルで開けます。対象種別の変更は項目フォームの選択肢へ即時反映されます。
...
- 標準／コンパクトの表示密度を切り替えられます。切替時は仮想行を再計測し、プロジェクトの初期密度と反対方向へ変更しても行位置を維持します。ズーム、スクロール、パン状態、密度はZustandのワークスペース単位ストアで管理します。
