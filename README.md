...
- 行表示、コンパクトレーン表示、Notion風テーブル表示、パン、11段階ズーム、横幅に応じた自動目盛り、下端に表示するホバー年月日、fit-range
- 時代区分・王朝・政権・文化潮流を重ねる年代背景レイヤー、曖昧端点グラデーション、重複警告、公開表示
- ズームを追加操作と表示モードの間へ置き、密度・1本の二点レンジによる期間強調を「表示」へ統合した一行のタイムライン上部ツールバー、アプリ内最大化／Fullscreen、垂直年代線、年代ジャンプ、位置ブックマーク
...
- 日本語の既定種別と自由入力に対応するアイテム／イベント間の意味的関係、直線／二重線、始点／終点矢印、半透明灰色・着色・非表示を選べる直角線、表示範囲を横切る線の継続描画、固定列との重なり順制御、リンク付き詳細一覧、編集・新規パネルからの関係登録
- タイムラインを直角四角、イベントを角丸四角で区別し、タイトルに応じた横172〜250px・固定高・最大2行省略表示を行う関連ネットワーク。意味的関係を設定どおりの方向・矢印・単線／二重線で描き、Force初期配置、重なり抑制、タグ・種別・年代・関係種別・検索、直接／2段階強調、段階クラスタ、ホイールズーム、パン、ノード一時移動、詳細遷移に対応
- 内部リンク切れ、削除済み参照、親・期間・出典・本文・必須項目・URL・未使用マスタ・関係・Markdownを対象ごとに理由集約するオンデマンド品質診断、複合条件の重複候補、参照を保った統合プレビューと期限付きUndo
- 統計・品質チェック、分類・関係、出典と、年代背景・設定・入出力を用途別に整理したキーボード操作対応の管理メニュー
- DB・JSON・CSVの現行版v7、旧JSON／CSVの段階的な自動移行、未知版の安全な拒否
- タイムラインアイテム／イベントアイテムの共通参照、Feature Flag、保持上限の共通基盤
- 保存単位の変更履歴、削除行／追加行の差分比較、特定版復元、手動チェックポイント
- 安全なMarkdown本文、GFM表・打ち消し線、5種のコールアウト、編集／プレビュー切り替え、内部リンク解決時に原文をちらつかせない描画、Markdown記法と主要機能別の静的ヘルプ
...
- ページ遷移後もlocalStorageで保持する左ナビゲーションの開閉状態と、下部に常設するヘルプ導線
...
- プロジェクト見出しの「他プロジェクトと比較」から複数の所有・公開プロジェクトを検索して上下比較。最大4画面を表示領域内に分割し、5画面目以降は縦スクロール、全画面の年代・パン・ズーム・ホバー線を同期。年代ヘッダーとツールバーは最上段だけに表示し、比較順は選択モーダルでドラッグ変更
- 「統計・品質チェック」パネルの統計／品質タブ。D3.jsによる種別・タグ・年代・期間・日付精度・関係・不足情報の可視化、直近1年の日別作成ヒートマップ、フィルター／期間強調連動、CSV出力
...
- 公開：保留中の下書きと公開版の分離
- ローカル利用：未ログインのルートページ、年・年月・年月日入力、IndexedDB自動保存、ローカル全文検索、JSON・CSV入出力、クラウド選択取込（PWAは提供しない）
- Windows版：ローカルDB、画像、長期履歴を扱う独立製品系列

## 技術構成

- Next.js App Router / React / TypeScript / pnpm
- Tailwind CSS / shadcn/ui / Radix UI / D3.js / jsPDF
...
- 所有者タイムラインの補助データはTanStack Queryの専用Hookから並列取得し、取得中は固定領域のスケルトンを表示する。公開タイムラインは初期HTMLを維持するためサーバー取得を継続する
- 管理メニュー配下の画面は操作時に遅延ロードし、初期クライアントバンドルから分離する
