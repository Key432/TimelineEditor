...
- 30日保持のゴミ箱、アイテムと子イベントの一括復元、明示完全削除、Supabase Cronによる期限切れ削除
...
- 履歴は90日・1エンティティ20版・1プロジェクト25MBを固定上限として古い版から自動整理し、画面には保持条件や容量を表示しない
- ゴミ箱は30日保持し、Supabase Cronの日次処理で期限切れデータを完全削除する
