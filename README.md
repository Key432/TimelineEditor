...
- プロジェクト削除時は名前の再入力が必要で、設定を含む配下データは `ON DELETE CASCADE` で完全削除されます。タイムラインの設定パネルから削除する間は画面操作をブロックし、完了後にプロジェクト一覧へ移動します。
...
- 保存前は破線の仮マーカー、保存後は輪郭を強調できるマーカーを表示します。タイムラインアイテムとイベントアイテムは共通ホバーUIでタイトルと日付または期間を固定ラベルなしに表示し、同じ輪郭強調を使用します。親タイムラインアイテムの強調中もイベントマーカーは前面に残り、イベントへホバーすると強調対象がイベントへ切り替わります。クリックは詳細、ダブルクリックは編集を開きます。
...
- タイムラインアイテム／イベントアイテムの編集保存時は詳細キャッシュを更新するため、`/edit` を閉じると通常詳細ページと詳細オーバーレイの双方へ変更が反映されます。URL連動モーダルから完全削除した場合は一覧キャッシュを更新してモーダルslotを閉じ、削除結果を背景のタイムラインへ即時反映します。イベントの完全削除中は画面操作をブロックします。
