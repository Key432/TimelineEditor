...
- タイムラインアイテム追加サイドパネルの時間形式、曖昧状態、表示状態、個別色、日付入力を再配置し、個別色にはカラーピッカーを追加しました。期間／時点を切り替えても入力済みの開始日を維持します。
...
- 詳細モーダルの全画面化、両個別ページ共通の余白切替、検索結果へ戻れるパンくずと安全な `returnTo` を追加しました。URL直接アクセス時もタイムラインへの導線を常設します。
