...
- 「表示」「画面」「移動・ビュー」に分類したタイムラインツールバー、アプリ内最大化／Fullscreen、垂直年代線、丸型アイコンから展開するフロート操作群、初期状態が非表示の期間強調、年代ジャンプ、位置ブックマーク
...
- 所有プロジェクトと閲覧可能な公開プロジェクトを最大6件まで同じ時間軸に並べる閲覧専用の横断比較。共通パン・ズーム・期間強調、プロジェクト表示切替、タグ・タイムライン種別・イベント種別フィルター、詳細を開いた後の比較状態保持、設定JSONだけの保存済み比較ビューに対応
