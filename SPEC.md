...
`POST /api/projects/[projectId]/items` は、タイムラインアイテムと同時作成する0件以上のイベントアイテムを任意で受け取れる。親タイムラインアイテムのINSERTとイベントアイテムのINSERTは、RLSが適用されるDB関数を介して単一トランザクションで実行する。イベントを含む場合は期間型の親だけを許可し、いずれか1件でも入力不正、権限不正、DB制約違反があれば親を含む全INSERTをロールバックする。
...
- 「おおよそ」「対象種別の色を上書き」「タイムラインに表示」等の真偽値は、ラベル、短い補足、操作部品を同じ設定行へまとめ、色だけに依存せず状態を示す
...
タイムラインアイテムの新規作成時は、同じサイドパネル内で0件以上のイベントアイテムを追加、編集、削除できる。イベント編集UIの項目順、日付入力、エラー表示は `/projects/[projectId]/items/[itemId]/edit` のイベント管理UIと共通化する。時間形式が時点型の間はイベント追加UIを無効化し、理由を表示する。保存時は親とイベントを単一リクエスト、単一DBトランザクションで作成する。
...
URL連動オーバーレイには、閉じるボタンと競合しない位置へ全画面化アイコンボタンを置く。通常の個別ページではタイムラインアイテムとイベントアイテムのコンテンツ最大幅、左右余白、ヘッダー操作位置を統一する。「左右の余白を縮小」オプションで標準の読みやすい幅と、最小限の安全余白を残した横長表示を切り替えられるようにする。この表示設定は一時的なUI設定として両詳細ページで共有し、データモデルには保存しない。
