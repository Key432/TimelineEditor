...
- タイムラインアイテムとイベントアイテムは依存関係・参照件数を確認後にゴミ箱へ移動し、保持期限内は復元可能
- ゴミ箱の保持期限経過後または明示的な完全削除で実体を削除
- 使用中の対象種別は直接削除不可
- タイムライン内の設定パネルからプロジェクトを完全削除する間は画面操作をブロックし、完了後に `/projects` へ置換遷移する
- イベントアイテムの詳細から完全削除する間は画面操作をブロックする。タイムラインから開いたURL連動モーダルでは一覧キャッシュ更新後に履歴を戻してモーダルslotを閉じ、直接詳細ページでは所属プロジェクトの `/timeline` へ置換遷移する
- タイムラインアイテムをURL連動モーダルから完全削除した場合も、項目・イベント一覧キャッシュ更新後に履歴を戻してモーダルslotを閉じ、削除結果を背景のタイムラインへ即時反映する
...
- 履歴、ゴミ箱、公開複製データは共通の保持上限処理を使用する。履歴の初期値は90日、1エンティティ20版、1プロジェクト25MBとし、公開版は現行版と過去最大1世代とする。ゴミ箱の具体的な期限は実装Phaseで必須指定する
...
- タイムラインアイテムとイベントアイテムの変更を差分履歴として保存する
- 変更前後の比較、特定版への復元、手動チェックポイントを提供する
- 一括操作は操作単位でUndoできる
- 連続入力を一定時間単位に統合し、文字入力ごとに履歴を作らない
- 保持期間、1エンティティの保持数、1プロジェクトの履歴上限を設定し、古い自動履歴から整理する
- ゴミ箱にも保持期限を設け、無制限履歴と長期スナップショットはWeb版で提供しない
- 完全な長期バックアップはJSONエクスポートを利用する
