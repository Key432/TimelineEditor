...
ユーザーが開始日、終了日、対象種別、説明、子イベント等を登録することで、横方向へ連続する時間軸上に年表を自動生成できるようにする。
...
- `temporal_type = point` の項目には子イベントを作成不可
...
- 仮想化の「表示中 n / n 行」はグループ見出しを除き、実際のタイムライン項目だけを数える
...
- タイムライン上の項目編集では別ページへ遷移せず、詳細情報を折りたたみ内で展開する
- 直接URLから開く編集ページでは詳細情報を初期展開する
- 長文説明
- 子イベント管理
- 出典
- 外部URL
