...
- 選択は両エンティティで共有し、ブラウザへローカル保存して再読み込み後も復元する
...
- 縮尺、表示単位、全体に合わせる、行密度の操作群はタイムラインフィールド右下へフロート表示する
- 時間スライサーはタイムラインフィールド内へフロート表示し、表示／非表示を切り替えられる。始点と終点を指定し、選択期間を着色して期間外をグレーアウトする。期間レイヤーは時間軸部分だけへ描画し、固定情報列と編集操作列を覆わない。全期間ミニマップは実装を保留する
...
- 現在の表示、時間スライサー、全範囲、任意年代範囲を出力できる
...
- パン、ズーム、時間スライサー、プロジェクト表示、タグ・種別フィルターを共有する
...
- 時間スライサー・フィルターと連動し、集計結果から該当項目へ移動し、CSV出力できる
